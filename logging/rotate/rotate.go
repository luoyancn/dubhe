...
func (r *Writer) SetMax(size int) {
	if size < minDefault || size > maximum {
		return
	}
	r.max = size
}
...
func (r *Writer) rotate() error {
	...
	filename := fmt.Sprintf("%s_%s", r.prefix, time.Now().Format("20060102150405000"))
	if err := os.Rename(path.Join(r.root, r.prefix), path.Join(r.root, filename)); err != nil {
		return err
	}
